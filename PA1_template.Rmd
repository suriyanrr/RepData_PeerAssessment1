...
The mean total number of steps per day: `r meanSteps`  
The median total number of steps per day: `r medianSteps`
...
The interval with the maximum number of steps: `r intervalMaxSteps`
...
There are `r missingData` missing values for "steps" in the dataset.
...
For the new dataset, the mean and median total number of steps per day are `r meanStepsImp` & `r medianStepsImp` respectively. This is different from the first part of the assignment. By imputing the missing data, the estimates of the number of steps have increased because values which were previously NA (and thus ignored in teh calcualtion) are now equal to the mean value. Hence, the mean and median values are slightly shifted from the previous values.
